  ...
  const statCards = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      <div className="bg-white rounded-2xl p-6 ring-1 ring-slate-200 shadow-sm card-hover">
        ...
        <Link href="/invoices" className="text-xs font-medium text-[#4B5563] hover:text-[#111827] hover:underline mt-3 inline-block transition-colors">View invoices →</Link>
      </div>
      ...
      <div className="bg-[#111827] rounded-2xl p-6 ring-1 ring-[#1F2937] shadow-sm card-hover">
        ...
        <Link href="/invoices" className="text-xs font-medium text-slate-400 hover:text-white hover:underline mt-3 inline-block transition-colors">View all →</Link>
      </div>
      ...
      <div className="bg-rose-50 rounded-2xl p-6 ring-1 ring-rose-100 shadow-sm card-hover">
        ...
        <Link href="/invoices?status=pending" className="text-xs font-medium text-rose-600 hover:underline mt-3 inline-block transition-colors">Collect dues →</Link>
      </div>
      ...
      <div className="bg-[#1F2937] rounded-2xl p-6 ring-1 ring-[#374151] shadow-sm card-hover">
        ...
        <Link href="/stock-in" className="text-xs font-medium text-slate-400 hover:text-white hover:underline mt-3 inline-block transition-colors">Pay suppliers →</Link>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 ring-1 ring-slate-200 shadow-sm card-hover">
        ...
        <Link href="/invoices" className="text-xs font-medium text-[#4B5563] hover:text-[#111827] hover:underline mt-3 inline-block transition-colors">View all →</Link>
      </div>
    </div>
  )
  ...
  const invoiceQuick = (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="bg-white rounded-2xl p-5 ring-1 ring-slate-200 shadow-sm flex items-center justify-between">
        ...
        <Link href="/invoices" className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#111827] bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg transition-colors">View →</Link>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 ring-1 ring-rose-100 shadow-sm flex items-center justify-between">
        ...
        <Link href="/invoices?status=pending" className="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 px-3 py-1.5 rounded-lg transition-colors">View →</Link>
      </div>
    </div>
  )
            ...
            {dueInvoices.map((inv) => {
              ...
              return (
                <tr key={inv.id} className="row-hover">
                  ...
                  <td className="px-5 py-3.5 text-xs text-slate-500 hidden sm:table-cell">{format(parseISO(inv.created_at), 'd MMM yyyy')}</td>
            ...
            <tbody className="divide-y divide-slate-100">
              {dueSupplierInvoices.map((inv) => (
                <tr key={inv.id} className="row-hover">
                  ...
                  <td className="px-5 py-3.5 text-xs text-slate-500 hidden sm:table-cell">{format(parseISO(inv.purchase_date), 'd MMM yyyy')}</td>
                  <td className="px-5 py-3.5 text-right">
